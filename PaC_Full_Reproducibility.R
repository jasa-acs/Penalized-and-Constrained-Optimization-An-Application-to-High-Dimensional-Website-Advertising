...
### For easier to use files depending on which section of the paper you intend to reproduce, please see
###     -- PaC_Simple_Table2.R for a walkthrough example of the code used to generate Tables 2 and 3
###     -- PaC_Tables_2_and_3.R for the code file run to generate the complete results for Tables 2 and 3
###     -- PaC_Timings_Fig2.R for the code file to run the timings plots given in Figure 2
...
### Section 5.3: Efficiency of PAC Algorithm
## Two settings used for comparison here:
## Setting 1 (n=100 and m=5) and Setting 3 (n=50 and m=60)
## Timings were generated using different values of 60
## Actual timings may vary from plot provided in Figure 2 due to machine used to run calculations
## Pattern should not change, however
## This uses only one run, so may get variable timings

## Setting 1

p.vector5=c(10,15,20,25,35,50,75,100,125,150,175,200,225,250,275,300,325,350,375,400,425,450,475,500)
n=100
m=5
times.5=rep(0,length(p.vector5))
times2.5=rep(0,length(p.vector5))
compare.5=vector("list", length(p.vector5))

for (i in 1:length(p.vector5)) {

  p=p.vector5[i]
  temp.data=generate.data(n=n,p=p,m=m,cov.mat=NULL)
  compare.5[[i]]=lasso.c(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,l.min=-2,l.max=6,verbose=F,intercept=F,normalize=T,backwards=F)
  times.5[i]=system.time(lasso.c(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,l.min=-2,l.max=6,verbose=F,intercept=F,normalize=T,backwards=F))[3]

  times2.5[i]=system.time(for (k in 1:length(compare.5[[i]]$lambda)) {
    quad.int(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,compare.5[[i]]$lambda[k])
  })[3]

}

times.5.corr=rep(0,length(p.vector5))
times2.5.corr=rep(0,length(p.vector5))
compare.5.corr=vector("list", length(p.vector5))
...
for (i in 1:length(p.vector5)) {
  ...
  for (l in 1:p) {
    for (o in 1:p) {
      if (l==o) {
        corr[l,o]=1 }
      else {corr[l,o]=0.5^abs(l-o)}
    }
  }
  
  cov.mat2=cor2cov(corr,rep(1,p))
  temp.data=generate.data(n=n,p=p,m=m,cov.mat=cov.mat2)
  compare.5.corr[[i]]=lasso.c(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,l.min=-2,l.max=6,verbose=F,intercept=F,normalize=T,backwards=F)
  times.5.corr[i]=system.time(lasso.c(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,l.min=-2,l.max=6,verbose=F,intercept=F,normalize=T,backwards=F))[3]
  times2.5.corr[i]=system.time(for (k in 1:length(compare.5.corr[[i]]$lambda)) {
    quad.int(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,compare.5.corr[[i]]$lambda[k])
  })[3]
...
p.vector60=c(65,70,85,100,125,150,175,200,225,250,275,300,325,350,375,400,425,450,475,500)
n=50
m=60
times.60.corr=rep(0,length(p.vector60))
times2.60.corr=rep(0,length(p.vector60))
compare.60.corr=vector("list", length(p.vector60))

for (i in 1:length(p.vector60)) {
  ...
  cov.mat2=cor2cov(corr,rep(1,p))
  temp.data=generate.data(n=n,p=p,m=m,cov.mat=cov.mat2)
  compare.60.corr[[i]]=lasso.c(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,l.min=-2,l.max=6,verbose=F,intercept=F,normalize=T,backwards=F)
  times.60.corr[i]=system.time(lasso.c(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,l.min=-2,l.max=6,verbose=F,intercept=F,normalize=T,backwards=F))[3]
  times2.60.corr[i]=system.time(for (k in 1:length(compare.60.corr[[i]]$lambda)) {
    quad.int(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,compare.60.corr[[i]]$lambda[k])
  })[3]
}

times.60=rep(0,length(p.vector60))
times2.60=rep(0,length(p.vector60))
compare.60=vector("list", length(p.vector60))

for (i in 1:length(p.vector60)) {

  p=p.vector60[i]
  temp.data=generate.data(n=n,p=p,m=m,cov.mat=NULL)
  compare.60[[i]]=lasso.c(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,l.min=-2,l.max=6,verbose=F,intercept=F,normalize=T,backwards=F)
  times.60[i]=system.time(lasso.c(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,l.min=-2,l.max=6,verbose=F,intercept=F,normalize=T,backwards=F))[3]
  times2.60[i]=system.time(for (k in 1:length(compare.60[[i]]$lambda)) {
    quad.int(temp.data$x,temp.data$y,temp.data$C.full,temp.data$b,compare.60[[i]]$lambda[k])
  })[3]

}

lambda5=rep(0,length(p.vector5))
lambda5.corr=rep(0,length(p.vector5))

for(i in 1:length(p.vector5)){
  
  lambda5[i]=length(compare.5[[i]]$lambda)
  lambda5.corr[i]=length(compare.5.corr[[i]]$lambda)
  
}


lambda60.corr=rep(0,length(p.vector60))
lambda60=rep(0,length(p.vector60))

for(i in 1:length(p.vector60)){
  
  lambda60.corr[i]=length(compare.60.corr[[i]]$lambda)
  lambda60[i]=length(compare.60[[i]]$lambda)
...
## Figure 2
## Note that your Figure 2 will likely look different than given Figure 2, depending on what machine you've used to run the code above
## Relative rankings of methods should not change, however
## You may notice your plot is much more jagged than in the PAC paper
## You can eliminate this by re-running the above code with more grid points
## Or re-running the code above and taking averages.
## However, this is not necessary to see the pattern and will take considerable
## time to run, so the single run has been left in the code for reproducibility
## purposes, although the smoother version of the plot is included as Figure 2.

par(mfrow=c(1,2),xpd=TRUE)

plot(p.vector5,log(times2.5.corr/lambda5.corr),type="l",col="red",xlab="Number of Coefficients (p)",ylab="log(time) (in seconds, per lambda)")
lines(p.vector5,log(times2.5/lambda5),lty=2,col="purple")
lines(p.vector5,log(times.5.corr/lambda5.corr),col="blue",lty=4)
lines(p.vector5,log(times.5/lambda5),lty=3)

plot(p.vector60,log(times2.60.corr/lambda60.corr),type="l",col="red",xlab="Number of Coefficients (p)",ylab="log(time) (in seconds, per lambda)",ylim=c(-5.5,3.5))
lines(p.vector60,log(times2.60/lambda60),lty=2,col="purple")
lines(p.vector60,log(times.60.corr/lambda60.corr),col="blue",lty=4)
lines(p.vector60,log(times.60/lambda60),lty=3)
